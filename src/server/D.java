...
public class D extends Thread {
	...
	public static final int numCadenas = 14;
	private static File fileCSV;
	private static boolean safety;
	
	public static void init(X509Certificate pCertSer, KeyPair pKeyPairServidor, File pFile, File pFileCSV, boolean pSafety) {
		safety = pSafety;
		certSer = pCertSer;
	...
	public void serverSeguro()
	{
	    ...
	    System.out.println(dlg + "Empezando atencion.");
	        try {
				...
				cpuLoadStart = getProcessCpuLoad();
				...
				trTime = endTime - starTime;
				double cpuLoadEnd = getProcessCpuLoad();
				
				String escribir = dlg + ";" + trTime + ";" + cpuLoadStart + ";" + cpuLoadEnd + ";" + result;
		        sc.close();
			    ...
			    synchronized (fileCSV) {
			    	escribirMensaje(escribir, fileCSV);
				}
	...
	public void serverInseguro()
	{
	    ...
	    System.out.println(dlg + "Empezando atencion.");
	        try {
				...
				/***** Fase 5: Envia llave simetrica *****/
				SecretKey simetrica = S.kgg(algoritmos[1]);
				byte[] simetric = simetrica.getEncoded();
				ac.println(toHexString(simetric));
				cadenas[7] = dlg +  ENVIO + "llave K_SC al cliente. continuado.";
				System.out.println(cadenas[7]);
